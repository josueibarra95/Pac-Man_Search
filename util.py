...
def closestManhDistPoint(start, points):
    """Finds closest point from points, according to the Manhattan Distance from start to the point."""
    if len(points) == 0:
        return ()

    closestManhDistPoint = points[0]
    for point in points:
        if manhattanDistance(start, point) < manhattanDistance(start, closestManhDistPoint):
            closestManhDistPoint = point
